...
export function normalizeAndSortTransfers(transfers: any[]): EthscriptionTransfer[] {
  return (
    transfers
...
export function normalizeResult(result: any, options?: any): EthscriptionBase {
  ...
  return withRes.length > 0 ? (withRes as typeof withRes) : (onlyRes as typeof onlyRes);
}

export function filtersNormalizer(opts: Record<string, any>): Record<string, any> {
